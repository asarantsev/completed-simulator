...
def simReturns(initVol, initRate, initSpread, initBubble, initEarn, nYears, bondPstart, bondPend, intlP):
    ...
    for t in range(nYears):
        ...
        simSpread[:, t + 1] = -0.263745 * np.ones(NSIMS) + 0.528863 * simSpread[:, t] + 0.502590 * simLRate[:, t] + noiseSpread[:, t]
        simLRate[:, t + 1] = 0.151941 * np.ones(NSIMS) - 0.043837 * simSpread[:, t] + (1 - 0.049088) * simLRate[:, t] + noiseLRate[:, t]
